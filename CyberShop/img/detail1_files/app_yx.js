...
console.log(v2);
								var typeHtml = '';
								if (v2.percent == 100 || v2.amount == 0) {
									typeHtml += "Tặng <span>" + v2.apply_quantity + "</span> <span>" + v.product_name + "</span>. ";
								} else {
									typeHtml += "Giảm giá <span>" + (v2.percent != null ? (v2.percent + "%") : Haravan.formatMoney(parseFloat(v.amount) * 100, "")+"₫") + "</span> sản phẩm " + v.product_name + ". ";
								}
								 html += `
								<tr haravan-promotion>
								<td>
								<input class="radio-product" data-percent="${v2.percent}" data-countbuy="${v2.quantity}" data-countgift="${v2.apply_quantity}" ${i == 0 ? checked = 'checked':checked = ''} type="radio" data-id="${v2.variant_id}" name="group-product-sale">
								</td>
								<td>
								...
								<p>
								${typeHtml} Khi mua <span> ${v2.quantity} </span><span> ${title}</span>
								</p>
								</a>
								</div>
								</td>
							...
							if (v.percent == 100 || v.amount == 0) {
								...
								typeHtml += "Giảm giá <span>" + (v.percent != null ? (v.percent + "%") : Haravan.formatMoney(parseFloat(v.amount) * 100, "")+"₫") + "</span> sản phẩm " + v.product_name + ". ";
							}
...
$('#buyxgety-product-list > table').html(html);
